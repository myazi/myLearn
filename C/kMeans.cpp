...
Matrix randCent(Matrix x,int kNum)
{
    int i,j,k;
    Matrix initmeans;
    initmeans.initMatrix(kNum,x.row,0,"ss");
...
CenAndDis kMeans(Matrix &x,const int &kNum, const int &Iter)
{
    ...
    */
    CenAndDis cendis;
    cendis.cen.initMatrix(kNum,x.row,0,"b");
...
int biKmeans(Matrix &x,const int &kNum,const int &Iter)
{
    int i,j,k,d;
    Matrix kmeans;
    kmeans.initMatrix(1,x.col,0,"ss");///初始化聚为一类，
