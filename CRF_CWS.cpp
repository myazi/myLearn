...
namespace MLL
{
	...
	int CRF_CWS::senToword(const std::string &sen,std::string *word)
	{
		int i = 0, j = 0, k = 0;
		int num = sen.size();
		std::string qj[QJBJ]= {"０","１","２","３","４","５","６","７","８","９","．"};
	...
	int CRF_CWS::stateToindex(const int &state1, const int &state2)
	{
	...
	int CRF_CWS::stateToindex3(const int &state1, const int &state2, const int &state3)
	{
	...
	void CRF_CWS::createVocabList(const DataStr &data)
	{
		std::ofstream ofile;
	...
	void CRF_CWS::bdzm(const std::string &pre_word, const std::string &next_word,double *word_times)
	{
		int n,j,m;
		for(n=0; bd[n]!=""; n++)
		{
	...
	int CRF_CWS::FenCi(DataStr &testdata)
	{
		int i,j,l,m,n,k;
		int pos1,pos2,pos3;
		std::string stateall;
	...
	int CRF_CWS::PingCe(const DataStr &resultdata,const DataStr &golddata)
	{
		int i,j,k;
	...
	CRF_CWS::CRF_CWS(const std::string &file)
	{
